...
class InvoiceProcessor(BaseDocumentProcessor):
    """Processes bank invoice documents."""
    ...
    def extract_fields(self, ocr_result: OCRResult) -> Dict[str, Any]:
        ...
        """
        return self.extractor.extract_invoice_fields(ocr_result)
    ...
    def validate_fields(self, fields: Dict[str, Any]) -> Dict:
        ...
        """
        validation_results = self.semantic_validator.validate_invoice_fields(fields)
        
        # Get field weights
        field_weights = self.config.get('scoring', {}).get('field_weights', {}).get('invoice', {})
    ...
    def validate_layout(self, ocr_result: OCRResult, image_shape: tuple) -> Dict:
        ...
        """
        return self.layout_validator.validate_invoice_layout(ocr_result, image_shape)
    
    def check_consistency(self, fields: Dict[str, Any]) -> Dict:
        """Check invoice field consistency.
        ...
        """
        consistency_checks = self.consistency_validator.validate_invoice_consistency(fields)
    ...
    def get_required_fields(self) -> list:
        """Get list of required fields for invoices.
        
        Returns:
            List of required field names
        """
        return self.config.get('semantic', {}).get('invoice', {}).get('required_fields', [])
